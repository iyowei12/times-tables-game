...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center font-sans overflow-hidden">
      ...
      <div className="shape w-96 h-96 bg-indigo-300 top-[-10%] left-[-10%]" />
      <div className="shape w-80 h-80 bg-purple-300 bottom-[5%] right-[-5%] animation-delay-2000" />
      <div className="shape w-64 h-64 bg-pink-200 top-[40%] right-[10%] animation-delay-4000" />

      {/* Sound Toggle */}
      <button 
        onClick={() => setIsSoundEnabled(!isSoundEnabled)}
        className="fixed top-6 right-6 p-3 glass rounded-full hover:scale-110 transition-transform z-50 text-indigo-600"
      >
        ...
        {gameState === 'start' && (
          <ScreenWrapper key="start">
            <motion.div 
              ...
              className="mb-8 p-10 bg-indigo-100 rounded-[3rem] shadow-inner border-4 border-white"
            >
              <Star size={100} className="text-indigo-600 fill-indigo-500/20" strokeWidth={1.5} />
            </motion.div>
            <h1 className="text-6xl md:text-7xl font-black text-indigo-900 mb-4 tracking-tighter drop-shadow-sm">
              九九乘法<span className="text-indigo-600">大挑戰</span>
            </h1>
            <p className="text-xl text-slate-500 mb-12 font-medium max-w-md text-center">
              迎接挑戰，成為乘法大師！準備好在壓力下展現你的反應力了嗎？
            </p>
            <button 
              onClick={() => setGameState('settings')}
              className="btn-primary flex items-center gap-3 text-2xl group"
            >
        ...
        {gameState === 'settings' && (
          <ScreenWrapper key="settings">
            <div className="glass p-8 md:p-12 rounded-[2.5rem] w-full max-w-2xl text-center">
              <h2 className="text-4xl font-black text-slate-800 mb-10 flex items-center justify-center gap-3">
                <Settings className="text-indigo-500" /> 遊戲設定
              </h2>
              ...
              {mode !== 'practice' && (
                <motion.div 
                  ...
                  className="mb-10 p-6 bg-indigo-50/50 rounded-2xl"
                >
                  <label className="block text-sm font-bold text-indigo-900 mb-4 uppercase tracking-widest">
                    ...
                    {[3, 5, 10, 20].map(s => (
                      <button 
                        ...
                        className={cn(
                          ...
                          timeLimit === s ? "bg-indigo-600 border-indigo-600 text-white" : "bg-white border-indigo-100 text-indigo-400 hover:border-indigo-300"
                        )}
                      >
          ...
          <ScreenWrapper key="playing">
            ...
            <div className="w-full flex justify-between items-center mb-8 px-4">
              <div className="flex items-center gap-4">
                <div className="glass px-6 py-3 rounded-2xl text-indigo-900 font-black text-xl flex items-center gap-2">
                  <Award className="text-amber-500" /> {score}
                </div>
                {combo > 1 && (
                <MotionDiv 
                  initial={{ scale: 0 }} animate={{ scale: 1 }}
                  className="bg-orange-500 text-white px-4 py-2 rounded-full font-black text-sm uppercase tracking-tighter"
                >
                  Combo x{combo}
                </MotionDiv>
                )}
              ...
              <div className="text-slate-400 font-bold text-lg">
                Question <span className="text-slate-800">{currentIndex + 1}</span> / 10
              </div>
            </div>

            {/* Timer Bar */}
            {mode !== 'practice' && (
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden mb-12 glass shadow-none">
                <MotionDiv 
                  className={cn(
                    "h-full transition-colors duration-500",
                    timeLeft < 3 ? "bg-rose-500" : "bg-indigo-500"
                  )}
                  initial={{ width: '100%' }}
            ...
            <div className="flex-1 flex flex-col items-center justify-center w-full">
              <div className="mb-5 flex flex-wrap items-center justify-center gap-3">
                <div className="glass px-4 py-2 rounded-full text-sm font-black tracking-[0.25em] text-indigo-500 uppercase">
                  {modeLabel}
                </div>
                {mode !== 'practice' && (
                  <div className={cn(
                    "px-4 py-2 rounded-full text-sm font-black tracking-[0.2em] uppercase",
                    timeLeft <= 3 ? "bg-rose-500 text-white shadow-lg shadow-rose-200" : "bg-white/90 text-slate-600 border border-slate-200"
                  )}>
              ...
              <div className="text-[8rem] md:text-[12rem] font-black text-indigo-900 leading-none flex items-baseline gap-4 md:gap-8 drop-shadow-2xl">
                <span>{currentQuestion?.num1}</span>
                <span className="text-indigo-400 text-6xl md:text-9xl">×</span>
              ...
              <MotionDiv 
                ...
                className={cn(
                  ...
                  !feedback && "text-indigo-800"
                )}
              >
                {userAnswer || <span className="text-indigo-100 opacity-50">?</span>}
            ...
            <div className="mt-12 glass p-4 rounded-[2.5rem] w-full max-w-md">
              <div className="grid grid-cols-3 gap-2">
                {[1, 2, 3, 4, 5, 6, 7, 8, 9, 'C', 0, 'DEL'].map((k) => (
                  <button
                    ...
                    className="h-16 bg-white hover:bg-slate-50 text-indigo-900 font-black text-2xl rounded-2xl shadow-sm transition-all active:scale-90 hover:-translate-y-0.5"
                  >
              ...
              <button 
                ...
                className="w-full mt-4 btn-primary py-6 text-2xl disabled:opacity-50 disabled:grayscale disabled:shadow-none"
              >
                CONFIRM
              </button>
            </div>
        ...
        {gameState === 'result' && (
          <ScreenWrapper key="result">
            <div className="glass p-10 md:p-16 rounded-[3rem] text-center w-full max-w-3xl relative overflow-hidden">
              <div className="absolute top-0 inset-x-0 h-4 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500" />
              
              <div className="text-[120px] mb-4">
                {score === 100 ? '👑' : score >= 80 ? '🔥' : score >= 60 ? '👌' : '💪'}
              </div>
              
              <h2 className="text-5xl font-black text-slate-800 mb-2">挑戰結束</h2>
              <p className="text-2xl text-slate-400 mb-10 font-bold">
                {score === 100 ? '太強了！你是乘法之神！' : score >= 80 ? '令人驚嘆的表現！' : '還有進步的空間，加油！'}
              </p>

              <div className="flex flex-col md:flex-row gap-6 justify-center mb-12">
                <div className="bg-indigo-50 p-8 rounded-3xl flex-1">
                  <div className="text-sm font-bold text-indigo-400 uppercase tracking-widest mb-1">總分</div>
                  <div className="text-7xl font-black text-indigo-600">{score}</div>
                </div>
                <div className="bg-slate-50 p-8 rounded-3xl flex-1">
                  <div className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">完成時間</div>
                  <div className="text-7xl font-black text-slate-600">{totalTimeSpent.toFixed(1)}<span className="text-2xl ml-1 font-normal opacity-50">s</span></div>
                </div>
                <div className="bg-amber-50 p-8 rounded-3xl flex-1">
                  <div className="text-sm font-bold text-amber-500 uppercase tracking-widest mb-1">正確率</div>
                  <div className="text-7xl font-black text-amber-600">{accuracy}<span className="text-2xl ml-1 font-normal opacity-50">%</span></div>
                </div>
              </div>
              ...
              {history.length > 0 && (
                <div className="text-left mb-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {history.map((h, i) => (
                      <div key={i} className="bg-white/50 border border-slate-100 p-4 rounded-2xl flex justify-between items-center group hover:bg-white transition-colors">
                        <span className="font-black text-lg">{h.num1} × {h.num2} = <span className="text-emerald-600">{h.answer}</span></span>
              ...
              <div className="flex gap-4 justify-center">
                ...
                <button onClick={startGame} className="btn-primary px-12">再刷一次</button>
              </div>
            </div>
          </ScreenWrapper>
        )}
    ...
    <button 
      ...
      className={cn(
        ...
        active 
          ...
          : "bg-white border-transparent text-slate-400 hover:border-indigo-100 hover:text-slate-600"
      )}
    >
      <div className={cn(
        "p-4 rounded-2xl transition-colors",
        active ? "bg-white/20" : "bg-indigo-50 text-indigo-500 group-hover:bg-indigo-100"
      )}>
        {React.cloneElement(icon, { size: 32 })}
      </div>
      <div>
        <div className="font-black text-xl mb-1">{title}</div>
        <div className={cn("text-xs font-bold", active ? "text-indigo-100" : "text-slate-400")}>{desc}</div>
      </div>
    </button>
  );
}
